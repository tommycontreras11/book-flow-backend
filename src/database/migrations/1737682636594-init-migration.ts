...
export class InitMigration1737682636594 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`loan-managements\` (\`id\` int NOT NULL AUTO_INCREMENT, \`uuid\` varchar(36) NOT NULL, \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`deletedAt\` datetime(6) NULL, \`loan_number\` varchar(100) NOT NULL, \`date_loan\` varchar(255) NOT NULL, \`date_return\` varchar(255) NOT NULL, \`amount_day\` float(10,2) NOT NULL, \`quantity_day\` int NOT NULL, \`comment\` varchar(250) NOT NULL, \`employee_id\` int NOT NULL, \`book_id\` int NOT NULL, \`user_id\` int NOT NULL, \`request_id\` int NOT NULL, \`status\` enum ('BORROWED', 'RETURNED') NOT NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`ALTER TABLE \`loan-managements\` ADD CONSTRAINT \`FK_ee1b61a41abf55d63deec63f774\` FOREIGN KEY (\`employee_id\`) REFERENCES \`employees\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE \`loan-managements\` ADD CONSTRAINT \`FK_8cf182dd8e210940169a69b4ad8\` FOREIGN KEY (\`book_id\`) REFERENCES \`books\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`loan-managements\` DROP FOREIGN KEY \`FK_8cf182dd8e210940169a69b4ad8\``);
        await queryRunner.query(`ALTER TABLE \`loan-managements\` DROP FOREIGN KEY \`FK_ee1b61a41abf55d63deec63f774\``);
